...
const char *IANAEncodingCStringFromNSStringEncoding(NSStringEncoding encoding)
{
    ...
    return [( NSString*)ianaCharacterSetName UTF8String];
...
- (id)initWithHTMLData:(NSData *)data encoding:(NSStringEncoding)encoding error:(NSError **)error
{
    ...
    xmlDoc_ = xmlReadMemory((const char*)[data bytes], (int)[data length], baseURL, xmlEncoding, kGDataXMLParseOptions);
    if (xmlDoc_ == NULL) {
        if (error) {
            *error = [NSError errorWithDomain:@"com.google.GDataXML"
                                         code:-1
...
@implementation GDataXMLNode (NSObject)

+ (NSString *)parseWith:(GDataXMLElement *)pRootElement key:(NSString *)key
{
    NSArray *arrayStu = [pRootElement elementsForName:key];//找出标签名字为key的所有元素，保存到数组中,key通常有'head'和'body'2种
...
+ (NSDictionary *)getResultWith:(NSString *)content
{
    NSError *error = nil;
    GDataXMLDocument *pXMLdocument = [[GDataXMLDocument alloc] initWithHTMLString:content error:&error];
    //定义一个保存查找结果的对象，从根元素开始保存
    GDataXMLElement *pRootElement = [pXMLdocument rootElement];
    NSString *head = [self.class parseWith:pRootElement key:@"head"];
    NSString *body = [self.class parseWith:pRootElement key:@"body"];
    head = head ?: @"";
    body = body ?: @"";
...
+ (NSString *)getHead:(NSString *)content
{
    NSDictionary *dic = [[self class] getResultWith:content];
    NSString *result = dic[@"head"];
    return result;
}

+ (NSString *)getBody:(NSString *)content
{
    NSDictionary *dic = [self.class getResultWith:content];
    NSString *result = dic[@"body"];
    return result;
}
