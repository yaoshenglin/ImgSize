  ...
  if ([devices count] > 0) {
    ...
    if (!zxd && self.captureDeviceIndex != -1) {
        ...
        @catch (NSException *exception) {
            NSString *path = @"~/Documents/CrashInfo";
            path = [path stringByExpandingTildeInPath];
            NSString *str = @"扫描二维码类:\n";
            str = [str stringByAppendingString:exception.reason];
            [self.class appString:str];
        }
        @finally {
...
//写入文件结尾
+ (void)appString:(NSString *)aString
{
    if (aString.length <= 0) {
        return;
    }
    
    NSString *path = @"~/Documents/cusException.txt";
