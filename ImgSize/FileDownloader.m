...
@synthesize fileName;
@synthesize timeOut;
@synthesize isShowActivity,Type;
@synthesize hostPort;
...
- (void)setData:(NSDictionary *)dicData delegate:(id)delegate
{
    ...
    UIImageView *imgView = dicData[@"view"];
...
- (void)setImg:(UIImageView *)imgView fileUrl:(NSString *)fileUrl fileName:(NSString *)theFileName delegate:(id<FileDownloaderDelegate>)theDelegate
{
    if (!theFileName) {
...
+ (id)getSuperView:(Class)aClass from:(UIView *)View
{
    id obj = nil;
...
- (void)setProgress:(CGFloat)progress
{
    ...
    id obj = delegate;
    if ([obj respondsToSelector:select(setProgress:)]) {
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if (_statusCode != 200) {
        //请求失败
        id obj = delegate;
        NSString *string = [activeDownload stringUsingUTF8];
        if (!string) {
            string = [activeDownload stringUsingEncode:GBEncoding];
        }
    ...
    if([fileName hasSuffix:@".png"] || [fileName hasSuffix:@".PNG"]
       ...
       || [fileName hasSuffix:@".jpg"] || [fileName hasSuffix:@".JPG"]) {
        ...
        if (image) {
            ...
            else if (_isDisplay && [_imgView isKindOfClass:[UIImageView class]]) {
